...
{
	int rowidA=threadIdx.x,m=blockDim.x,sum,k;
	for (int colidB=0;colidB<q;colidB++)
	{
		sum=0;
		for (k=0;k<m;k++) sum=sum+a[rowidA*n+k]*b[k*q+colidB];
		c[rowidA*n+colidB]=sum;
	}
}
__global__ void matColMul(int *a,int *b,int *c,int m,int n)
{
	...
	{
		sum=0;
		for (k=0;k<q;k++) sum=sum+a[rowidA*n+k]*b[k*q+colidB];
		c[rowidA*n+colidB]=sum;
	}
}
